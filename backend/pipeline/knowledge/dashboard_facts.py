...
CUSTOM_COMPONENT_LIST = PageFacts(
    ...
    note=(
        ...
        '  The "Add a field in your Content Type" dialog (tile picker) auto-opens; it has NO Save button.\n'
        '  Per field: (a) click the type-tile HEADING, dialog-scoped + exact:\n'
        '      page.get_by_role("dialog").get_by_role("heading", name="Text", exact=True).click()\n'
        '      (exact=True is REQUIRED: "Text" also matches "Rich Text", "List" also matches "Dynamic List").\n'
        '      The dialog then RENAMES to a field-config form (no longer "Add a field in your Content Type").\n'
        '  (b) fill the field name: safe_sequential_fill(page, "Display Name", field_name) — the config-form\n'
        '      label is "Display Name" WITHOUT asterisk; "Name (Slug) *" auto-fills from it (do NOT fill it).\n'
        '  (c) set any TYPE-SPECIFIC required "*" config (see FIELD TYPES below) BEFORE saving.\n'
        '  (d) click "Add Another Field" for more fields, or on the last click the UNNAMED-dialog Save:\n'
        '      page.get_by_role("dialog").get_by_role("button", name="Save").click()\n'
        '      NEVER scope this Save to name="Add a field in your Content Type" — the dialog has renamed, so\n'
        '      that named locator matches nothing and times out.\n'
        '  After all fields: click the header Save: page.locator("#page-header").get_by_role("button", name="Save").click().\n'
        'FIELD TYPES — all 14 (verified live 2026-07-22). Every config form has: Display Name * (fill),\n'
        '  Name (Slug) * (auto-fills — skip), Description (optional), usually a "Make this field required" toggle.\n'
        '  EXTRA required "*" config you MUST set before the dialog Save (else Save never enables → timeout):\n'
        '    Numbers -> Type* (select: integer/float/decimal)\n'
        '    Date And Time -> Type* AND Date Format* (two selects)\n'
        '    List -> Select Type* (select) + List Choices* (textarea, comma-separated, e.g. "a, b, c")\n'
        '    Relation -> Content Type* (select, per-publisher) [+ One/Many Reference radio]\n'
        '    Dynamic List -> Related Model* (select, per-publisher) [no "required" toggle]\n'
        '    Component -> Select Component* (select, an EXISTING component) [+ Single/Repeatable radio]\n'
        '  Need ONLY Display Name (no extra required): Text, Media, Email, Rich Text, Boolean, JSON, Embed, Links.\n'
        '  For each required "*" select: click it, then pick page.get_by_role("dialog").locator(".ant-select-item-option").first\n'
        '  — never hardcode option text for the per-publisher selects (Relation Content Type, Dynamic List\n'
        '  Related Model, Component Select Component). Relation/Dynamic List/Component need a target that already\n'
        '  exists for this publisher; if none exists, use a simpler type (Text is safest).\n'
        ...
        'NOTE: "Display Name" appears in two contexts with DIFFERENT labels: Step 1 create dialog = '
        '"Display Name *" (with asterisk, component name); Step 2 field-config form = "Display Name" '
        '(no asterisk, field name). Fill each with the label shown for that context.'
    ),
)
